...
import "../IERC/IHoldableERC1400TokenExtension.sol";
...
contract ERC1400TokensValidator is IERC1400TokensValidator, Pausable, CertificateSignerRole, AllowlistedRole, BlocklistedRole, ERC1820Client, ERC1820Implementer, IHoldableERC1400TokenExtension {
    ...
    returns(bool)
  {
    if (_holdsActivated[token] && from != address(0)) {
      if(operator != from) {
        ...
        Hold storage holdToken = _holds[token][holdId];
        
        if (_holdCanBeExecutedAsNotary(holdToken, operator, value) && value <= IERC1400(token).balanceOfByPartition(partition, from)) {
          return true;
        }
      }
  ...
  ) internal returns (bool)
  {
    ...
    // @dev never return false due to function will be revert if canExcuteHold equal false,
    // add to ignore warning.
    return false;
  }
   ...
   */
  function _increaseHeldBalance(address token, Hold storage executableHold) private {
