...
import "../IERC/IERC1400.sol";

// Extensions
import "../IERC/IERC1400TokensValidator.sol";
import "../IERC/IERC1400TokensChecker.sol";
import "../IERC/IERC1400TokensSender.sol";
import "../IERC/IERC1400TokensRecipient.sol";
import "../Tools/DomainAware.sol";


/**
 * @title ERC1400 base logic
 * @dev ERC1400 logic
 */
contract ERC1400 is IERC20, IERC1400, Ownable, ERC1820Client, ERC1820Implementer, MinterRole, DomainAware {
  ...
  )
  {
    ...
    _setControllers(tokenControllers);
   ...
   */
  function allowance(address ownerAllowed, address spender) external override view returns (uint256) {
    return _allowed[ownerAllowed][spender];
  }
  /**
   ...
   * @param docName Short name (represented as a bytes32) associated to the document.
   * @return Requested document + document hash + document timestamp.
   */
  function getDocument(bytes32 docName) external override view returns (string memory, bytes32, uint256) {
    require(bytes(_documents[docName].docURI).length != 0); // Action Blocked - Empty document
    return (
      _documents[docName].docURI,
      _documents[docName].docHash,
      _documents[docName].timestamp
    );
  }

  /**
   * @dev Associate a document with the token.
   * @param docName Short docName (represented as a bytes32) associated to the document.
   * @param uri Document content.
   * @param documentHash Hash of the document [optional parameter].
   */
  function setDocument(bytes32 docName, string calldata uri, bytes32 documentHash) external override {
    require(_isController[msg.sender]);
    _documents[docName] = Doc({
      docURI: uri,
    ...
    emit DocumentUpdated(docName, uri, documentHash);
  }

  /**
   * @dev Dissociate a document with the token.
   * @param docName Short docName (represented as a bytes32) dissociate to the document.
   */
  function removeDocument(bytes32 docName) external override {
    require(_isController[msg.sender], "Unauthorized");
    require(bytes(_documents[docName].docURI).length != 0, "Document doesnt exist"); // Action Blocked - Empty document

    Doc memory data = _documents[docName];
    ...
    emit DocumentRemoved(docName, data.docURI, data.docHash);
  }

  /**
   * @dev Get all documents associated with the token.
   */
  ...
  function domainVersion() public override pure returns (bytes32) {
    return VERSION;
  }
  /************************************************************************************************/
}
